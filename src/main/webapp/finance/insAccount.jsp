					...
					<tr>
						...
						<td>
							<div id = "deleteButton" class = "toolButton" title = "delete">
								<a href = "<%=path %>/financeController/rejectInsByFinance.do?id=${ins.id }&currentPage=${pb.currentPage}">X</a>
							</div>
							<div id = "submitButton" class = "toolButton" title = "submit">
								<a href = "<%=path %>/financeController/submitInsByFinance.do?id=${ins.id }&currentPage=${pb.currentPage}">^</a>
							</div>
						</td>
					</tr>
				</c:forEach>
				...
				<tr>
					<td colspan="14">
						<c:if test="${pb.currentPage != 1 }">
							...
							<a href = "<%=path %>/financeController/getInsByPageAndStatus.do?currentPage=1">First Page &emsp;</a>
							<a href = "<%=path %>/financeController/getInsByPageAndStatus.do?currentPage=${pb.currentPage-1 }">Previous Page &emsp;</a>
						...
						<c:if test="${pb.currentPage != pb.totalPages }">
							...
							<a href = "<%=path %>/financeController/getInsByPageAndStatus.do?currentPage=${pb.currentPage+1 }">Next Page &emsp;</a>
							<a href = "<%=path %>/financeController/getInsByPageAndStatus.do?currentPage=${pb.totalPages }">Last Page &emsp;</a>
						</c:if>
